...
public class LsTest {
  ...
  public void testFiles() throws IOException {
    ...
    assertEquals(Arrays.asList("Control", "Ground", "Major", "Tom", "to"), executeAndGetList("ls"));
  }
  ...
  public void testFilesFolders() throws IOException {
    ...
    assertEquals(Arrays.asList("Files", "Some", "and", "folders"), executeAndGetList("ls"));
  }

  @Test
  public void lsFolder() throws IOException {
    final File folder = tmpFolder.newFolder("folder");
    final List<String> fileNames = Arrays.asList("fileA", "fileB", "fileC");
    for (String fn : fileNames) {
      while (!new File(folder, fn).createNewFile()) {
      }
    }

    assertEquals(Arrays.asList("fileA", "fileB", "fileC"), executeAndGetList("ls folder"));
  }

  private List<String> executeAndGetList(String cmd) throws IOException {
